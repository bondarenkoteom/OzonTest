...
public class AllureCucumber5Jvm implements ConcurrentEventListener {
    ...
    private io.qameta.allure.model.Status translateTestCaseStatus(final Result testCaseResult) {
        switch (testCaseResult.getStatus()) {
            case FAILED:
                ...
                return getStatus(testCaseResult.getError())
                        .orElse(io.qameta.allure.model.Status.FAILED);
            case PASSED:
    ...
    private void handleHookStep(final TestStepFinished event) {
        ...
        if (!io.qameta.allure.model.Status.PASSED.equals(fixtureResult.getStatus())) {
            ...
            if (hookStep.getHookType() == HookType.BEFORE) {
                ...
                testResult.setStatus(io.qameta.allure.model.Status.BROKEN);
                updateTestCaseStatus(testResult.getStatus());
            }
    ...
    private void handlePickleStep(final TestStepFinished event) {
        ...
        final io.qameta.allure.model.Status stepStatus = translateTestCaseStatus(event.getResult());
        ...
        if (!io.qameta.allure.model.Status.PASSED.equals(stepStatus) && stepStatus != null) {
            forbidTestCaseStatusChange.set(true);
        }
    ...
    private void updateTestCaseStatus(final io.qameta.allure.model.Status status) {
